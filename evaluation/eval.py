...
def evaluate_notes_processor():
    ...
    if response.status_code != 200:
        print(f"[FAIL] Failed to process notes: {response.status_code} - {response.text}")
    ...
    if len(action_items) != 5:
        print(f"[FAIL] Expected 5 action items, found {len(action_items)}")
        passed = False
    else:
        print("[PASS] All 5 action items extracted")
    ...
    if "Unassigned" not in owners:
        print("[FAIL] Missing owner did not become 'Unassigned'")
        passed = False
    else:
        print("[PASS] Missing owner becomes 'Unassigned'")
    ...
    if unassigned_items and unassigned_items[0].get("deadline") is not None:
        print("[FAIL] Missing deadline did not become null")
        passed = False
    else:
        print("[PASS] Missing deadline becomes null")

    if duration > 10.0:
        print(f"[FAIL] Response time over 10 seconds: {duration:.2f}s")
        passed = False
    else:
        print(f"[PASS] Response time under 10 seconds: {duration:.2f}s")
...
def evaluate_email_summarizer():
    ...
    if response.status_code != 200:
        print(f"[FAIL] Failed to process email: {response.status_code} - {response.text}")
        return False, duration
    ...
    if not (action_1_found and action_2_found):
        print("[FAIL] Failed to extract correct action items from email.")
        print(f"   Found: {data.get('action_items')}")
        passed = False
    else:
        print("[PASS] Correct action items extracted from email")
...
def evaluate_qa():
    ...
    with open(os.path.join(EVAL_DIR, "sample_meeting_notes.txt"), "rb") as f:
        client.post(
            "/api/documents",
            files={"file": ("sample_meeting_notes.txt", f, "text/plain")},
        )
    ...
    if resp1.status_code != 200:
        print(f"[FAIL] QA error: {resp1.text}")
        return False, 0
    data1 = resp1.json()
    if not data1.get("found"):
        print("[FAIL] Failed to find known answer (grant deadline).")
        passed = False
    elif "April 1" not in data1.get("answer", ""):
        print("[FAIL] Found answer but it didn't contain 'April 1'.")
        passed = False
    elif "source: sample_meeting_notes.txt" not in data1.get("source_citation", ""):
        print("[FAIL] Missing or incorrect source citation format.")
        passed = False
    elif not data1.get("matched_text"):
        print("[FAIL] Missing matched text snippet.")
        passed = False
    else:
        print("[PASS] Correctly answered and cited known question")
    ...
    if data2.get("found") or "I couldn't find this" not in data2.get("answer", ""):
        print("[FAIL] Failed hallucination check (hiring).")
        passed = False
    else:
        print("[PASS] Correctly rejected unknown question")
...
def evaluate_retrieval_and_reranking():
    ...
    if deadline_chunk_index == -1:
        print("[FAIL] Retrieval Recall@10 Failed: Grant deadline chunk not found in top 10.")
        passed = False
    else:
        print(f"[PASS] Retrieval Recall@10 Passed (Ranked #{deadline_chunk_index + 1})")
    ...
    if reranked_deadline_chunk_index == -1:
        print("[FAIL] Re-Ranking Failed: Grant deadline chunk not found in top 3.")
        passed = False
    else:
        print(f"[PASS] Re-Ranking Improvement Passed (Ranked #{reranked_deadline_chunk_index + 1})")
...
def main():
    ...
    if llm_provider == "anthropic" and not os.getenv("ANTHROPIC_API_KEY"):
        print("[FAIL] ANTHROPIC_API_KEY is not set. Please set it to run Anthropic evaluations.")
        sys.exit(1)
    elif llm_provider == "gemini" and not os.getenv("GEMINI_API_KEY"):
        print("[FAIL] GEMINI_API_KEY is not set. Please set it to run Gemini evaluations.")
        sys.exit(1)

    print(f"[EVAL] Running evaluations using Provider: {llm_provider.upper()}")
